...
def check_all_files(**kwargs):
    ...
    user_streams = check_files_in_s3('data/user-streams/')
    songs = check_files_in_s3('data/songs/')
    users = check_files_in_s3('data/users/')
...
def move_files_to_archived(**kwargs):
    s3 = boto3.client('s3')
    bucket = 'spotify-song-data'
    source_prefix = 'data/user-streams/'
    dest_prefix = 'data/user-streams-archived/'
